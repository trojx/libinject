...
{
	int i = 0;
	for(; i < g_whitelist_length; ++i)
	{
		if (strcmp(whitelist[i], fp) == 0)
		{
			break;
		}
		if (strlen(whitelist[i]) == 0)
		{
			memcpy(whitelist[i], fp, 8);
			break;
		}
	}
}
void libinjection_sqli_pop_whitelist(const char* data)
{
		    ...
		    int flag_list[] = {FLAG_QUOTE_NONE | FLAG_SQL_ANSI, FLAG_QUOTE_NONE | FLAG_SQL_MYSQL, FLAG_QUOTE_SINGLE | FLAG_SQL_ANSI, FLAG_QUOTE_SINGLE | FLAG_SQL_MYSQL, FLAG_QUOTE_DOUBLE | FLAG_SQL_MYSQL};
		    libinjection_sqli_init(&sf, data, slen, 0);

		    for (i = 0; i < 5; ++i)
		    {
		    	libinjection_sqli_fingerprint(&sf, flag_list[i]);
		    	for(; j < g_whitelist_length; ++j)
		    	{
		    		if (strcmp(g_whitelist[j], sf.fingerprint) == 0)
		    		{
		    			memset(g_whitelist[j], 0 , 8);
		    		}
		    	}
		    }
}
void libinjection_sqli_put_whitelist(char* data)
{
	    ...
	    {
	    	libinjection_sqli_fingerprint(&sf, flag_list[i]);
	    	/*fprintf(stdout, "1111111111\t%s\t111111111\n", sf.fingerprint);*/
	    	insert_fp(g_whitelist, sf.fingerprint);
	    }
}
/***********************************************************/
...
int libinjection_sqli_not_whitelist(struct libinjection_sqli_state* sql_state)
{
   ...
   for(; i < g_whitelist_length; ++i)
    {
    	if (strcmp(g_whitelist[i], sql_state->fingerprint) == 0)
    	{
    		return FALSE;
    	}
    }
